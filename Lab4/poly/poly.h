...
namespace coen79_lab4
{
    ...
    public:
      ...
      //CONSTRUCTOR
      polynomial(double c = 0.0, unsigned int exp = 0);

      //MODIFICATION MEMBER FUNCTIONS
      void assign_coef(double c, unsigned int exp);
      void add_to_coef(double amount, unsigned int exp);
      void clear();

      //CONSTANT MEMBER FUNCTIONS
      polynomial antiderivative() const;
      double coefficient(unsigned int exp) const;
      double definite_integral(double x0, double x1) const;
      unsigned int degree() const;
      polynomial derivative() const;
      double eval(double x) const;
